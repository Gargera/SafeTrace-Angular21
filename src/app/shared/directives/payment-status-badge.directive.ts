  ...
  ngOnChanges(): void {
    ...
    this.badgeRenderService.updateBadge(this.el.nativeElement, this.status as PaymentStatus, {
      ...
      getClasses: (val: PaymentStatus) => {
        if (val === PaymentStatus.Succeeded)
      ...
      getContent: (val: PaymentStatus) => {
        ...
        if (val === PaymentStatus.Succeeded) {
          ...
          translation = 'قيد الانتظار';
          icon = 'hourglass_empty';
        } else if (val === PaymentStatus.Failed) {
          translation = 'فشل';
          icon = 'cancel';
        } else if (val === PaymentStatus.Cancelled) {
          translation = 'ملغي';
          icon = 'block';
        } else if (val === PaymentStatus.Refunded) {
          translation = 'مسترد';
          icon = 'undo';
        }
