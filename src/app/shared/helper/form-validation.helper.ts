...
export function getControlFieldError(control: AbstractControl | null): string | null {
  ...
  if (e['min']) {
    if (e['min'].min === 1) return 'العمر يجب أن يكون أكبر من صفر';
    return `يجب أن لا تقل القيمة عن ${e['min'].min}`;
  }
  ...
  if (e['urgentTooOld']) return 'تاريخ ووقت الحالة يجب ألا يتجاوز 24 ساعة';
  ...
  if (e['maxFilesCount']) return `يمكنك إضافة ${e['maxFilesCount']?.max ?? 4} صور إضافية كحد أقصى`;
  if (e['rejectionReason']) return 'يجب ألا يقل سبب الرفض عن 10 حروف وألا يتجاوز 500 حرف';
  if (e['invalidCity']) return 'المدينة المحددة غير صحيحة';
